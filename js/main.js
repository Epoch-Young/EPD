...
async function preConnect(name_filter) {
...
function convertDithering() {
  ...
  const rawValue = parseFloat(document.getElementById('ditherStrength').value);
  let dithering =Math.pow(rawValue, 3);
  const mode = document.getElementById('ditherMode').value;
  const Color_enabled = document.getElementById('colorCheck');
  
    if(mode === 'fourColor'&&Color_enabled.checked){

    grayscaleImageData = ditherTo4Grayscale(adjustedData, dithering);
    finalImageData = applyRedYellowEffect(
      new ImageData(
  ...
  finalImageData = decodeProcessedData(processedData, canvas.width, canvas.height, mode);
}
