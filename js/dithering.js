...
//
function convertBWimage(imageData){
  ...
  if (isGrayscale) {
    for (let i = 0; i < data.length; i += 4) {
      const grayValue = data[i]; // 取R值即可，因为是灰度图
      // 将灰度值映射到四色
      if (grayValue < 64) {
        // 黑色
...
function applyBrightnessContrast(imageData, brightness, factor) {
  const data = imageData.data;
  for (let i = 0; i < data.length; i += 4) {
...
function ditherTo4Grayscale(imageData, ditheringAmount) {
  const width = imageData.width;
  const height = imageData.height;
  const data = imageData.data;
  ...
  for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
          ...
          // 添加随机抖动
          const ditheredGray = gray + (Math.random() - 0.5) * 255 * ditheringAmount;
          ...
          for (let l = 0; l < levels.length; l++) {
              const diff = Math.abs(ditheredGray - levels[l]);
              if (diff < minDiff) {
                  minDiff = diff;
                  closestLevel = l;
              }
          ...
          // 设置灰度值
          const val = levels[closestLevel];
          result.data[i] = result.data[i+1] = result.data[i+2] = val;
          result.data[i+3] = 255; // Alpha通道
      }
  }
  
  return result;
}
function applyRedYellowEffect(imageData) {
    const data = imageData.data;
    
    for (let i = 0; i < data.length; i += 4) {
        const grayValue = data[i]; // R=G=B
        
        if (grayValue === 85) { // 深灰 -> 红色
            data[i] = 186;   // R
            data[i+1] = 0;   // G
            data[i+2] = 0;   // B
        } else if (grayValue === 170) { // 浅灰 -> 黄色
            data[i] = 255;   // R
            data[i+1] = 204; // G
            data[i+2] = 4;   // B
        }
...
function rotateImage(imageData) {
  ...
  for (let y = 0; y < height; y++) {
    for (let x = 0; x < width; x++) {
      ...
      // 复制RGBA四个通道的值
      newData[newIndex] = data[originalIndex];         // R
      newData[newIndex + 1] = data[originalIndex + 1]; // G
      newData[newIndex + 2] = data[originalIndex + 2]; // B
      newData[newIndex + 3] = data[originalIndex + 3]; // A
    }
  }
